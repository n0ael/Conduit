...
| `/live/song/set/midi_input_focus` | `track_ref, s input, s default_input` — Ziel-MIDI-Track: Monitor „In" + Input `input`; alle anderen MIDI-Tracks „Auto" + `default_input` (leer = Routing unangetastet); Match exakter `display_name`, sonst Präfix case-insensitive (Conduit Grid-Track-Selector, Block H) |
