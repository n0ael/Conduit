...
namespace
{
    constexpr ParameterInfo kParameters[] = {
        ...
        { "derez",    "Derez",   0.5f },
        { "filter",   "Filter",  0.25f },
        { "early_rf", "EarlyRF", 0.5f },
        { "positin",  "Positin", 0.75f },
        { "dry_wet",  "Dry/Wet", 0.5f },
    };
}

KWoodRoom::KWoodRoom() noexcept
    : AirwindowsPlugin ("kwoodroom", "kWoodRoom", kParameters)
{
}

void KWoodRoom::resetState() noexcept
{
...
void KWoodRoom::processStereo (const float* in1, const float* in2,
                               float* out1, float* out2, int sampleFrames) noexcept
{
    ...
    double reg6n = (1.0 - std::pow (1.0 - A, 1.618033988749894)) * fdb6ck;
    // Start hiermit, aber im Loop nach Ausgangslautstärke dämpfen?

    double derez = B * 2.0;
    bool stepped = true; // überarbeitetes Bezier-Undersampling
    if (derez > 1.0) // volle Auflösung in der Mitte, gestuft
    {
        stepped = false; // nach links, kontinuierlich nach rechts
        derez = 1.0 - (derez - 1.0);
    } // wenn so eingerichtet, ist es der überarbeitete Algorithmus
    ...
    if (stepped) // sperrt derez hart auf exakte Unterteilungen von 1.0
    {
        derez = 1.0 / bezFraction;
        bezTrim = 1.0 - (derez * bezTrim);
    }
    else // damit passt es zum 1.0-Fall wie im gestuften Zweig
    {
    ...
    bool steppedFreq = true; // überarbeitetes Bezier-Undersampling
    if (derezFreq > 1.0) // volle Auflösung in der Mitte, gestuft
    {
        steppedFreq = false; // nach links, kontinuierlich nach rechts
        derezFreq = 1.0 - (derezFreq - 1.0);
    } // wenn so eingerichtet, ist es der überarbeitete Algorithmus
    derezFreq = std::fmin (std::fmax (derezFreq, 0.0005), 1.0); // kein overallscale — bereits im Undersampling
    int bezFreqFraction = (int) (1.0 / derezFreq);
    double bezFreqTrim = (double) bezFreqFraction / (bezFreqFraction + 1.0);
    if (steppedFreq) // sperrt derez hart auf exakte Unterteilungen von 1.0
    {
    ...
    double earlyLoudness = std::pow (D, 2.0);
    ...
    while (--sampleFrames >= 0)
    {
        ...
        if (bez[bez_cycle] > 1.0) // Endpunkt erreicht -> ein Reverb-Sample
        {
            ...
            c6ZBR++; if (c6ZBR > d6ZB) c6ZBR = 0; // Original-Quirk: erhöht c6ZBR statt c6ZGR (1:1 übernommen)
