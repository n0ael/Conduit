 ...
 *
 *  Bekannte Original-Eigenheit (bewusst NICHT korrigiert, mechanischer
 *  Port): die zweite Bezier-Undersampling-Stufe (bezF) akkumuliert den
 *  rechten Kanal aus `inputSampleL` statt `inputSampleR`
 *  (`bezF[bez_SampR] += (inputSampleL + bezF[bez_InR]) * derezFreq`,
 *  identisch in processReplacing UND processDoubleReplacing des Originals
 *  — kein Tippfehler beim Portieren, Verhalten 1:1 übernommen).
 * ======================================== */
...
namespace
{
    constexpr ParameterInfo kParameters[] = {
        ...
        { "derez",   "Derez",   0.5f },
        { "filter",  "Filter",  0.24f },
        { "earlyrf", "EarlyRF", 0.5f },
        { "positin", "Positin", 0.5f },
        { "drywet",  "Dry/Wet", 0.5f },
    };
}

KCathedral5::KCathedral5() noexcept
    : AirwindowsPlugin ("kcathedral5", "kCathedral5", kParameters)
{
}

void KCathedral5::resetState() noexcept
{
    std::fill (std::begin (a3AL), std::end (a3AL), 0.0); std::fill (std::begin (a3AR), std::end (a3AR), 0.0);
    std::fill (std::begin (a3BL), std::end (a3BL), 0.0); std::fill (std::begin (a3BR), std::end (a3BR), 0.0);
    std::fill (std::begin (a3CL), std::end (a3CL), 0.0); std::fill (std::begin (a3CR), std::end (a3CR), 0.0);
    std::fill (std::begin (a3DL), std::end (a3DL), 0.0); std::fill (std::begin (a3DR), std::end (a3DR), 0.0);
    std::fill (std::begin (a3EL), std::end (a3EL), 0.0); std::fill (std::begin (a3ER), std::end (a3ER), 0.0);
    std::fill (std::begin (a3FL), std::end (a3FL), 0.0); std::fill (std::begin (a3FR), std::end (a3FR), 0.0);
    std::fill (std::begin (a3GL), std::end (a3GL), 0.0); std::fill (std::begin (a3GR), std::end (a3GR), 0.0);
    std::fill (std::begin (a3HL), std::end (a3HL), 0.0); std::fill (std::begin (a3HR), std::end (a3HR), 0.0);
    std::fill (std::begin (a3IL), std::end (a3IL), 0.0); std::fill (std::begin (a3IR), std::end (a3IR), 0.0);
    c3AL = c3BL = c3CL = c3DL = c3EL = c3FL = c3GL = c3HL = c3IL = 1;
    c3AR = c3BR = c3CR = c3DR = c3ER = c3FR = c3GR = c3HR = c3IR = 1;

    std::fill (std::begin (a6AL), std::end (a6AL), 0.0); std::fill (std::begin (a6AR), std::end (a6AR), 0.0);
    std::fill (std::begin (a6BL), std::end (a6BL), 0.0); std::fill (std::begin (a6BR), std::end (a6BR), 0.0);
    std::fill (std::begin (a6CL), std::end (a6CL), 0.0); std::fill (std::begin (a6CR), std::end (a6CR), 0.0);
    std::fill (std::begin (a6DL), std::end (a6DL), 0.0); std::fill (std::begin (a6DR), std::end (a6DR), 0.0);
    std::fill (std::begin (a6EL), std::end (a6EL), 0.0); std::fill (std::begin (a6ER), std::end (a6ER), 0.0);
    std::fill (std::begin (a6FL), std::end (a6FL), 0.0); std::fill (std::begin (a6FR), std::end (a6FR), 0.0);
    std::fill (std::begin (a6GL), std::end (a6GL), 0.0); std::fill (std::begin (a6GR), std::end (a6GR), 0.0);
    std::fill (std::begin (a6HL), std::end (a6HL), 0.0); std::fill (std::begin (a6HR), std::end (a6HR), 0.0);
    std::fill (std::begin (a6IL), std::end (a6IL), 0.0); std::fill (std::begin (a6IR), std::end (a6IR), 0.0);
    std::fill (std::begin (a6JL), std::end (a6JL), 0.0); std::fill (std::begin (a6JR), std::end (a6JR), 0.0);
    std::fill (std::begin (a6KL), std::end (a6KL), 0.0); std::fill (std::begin (a6KR), std::end (a6KR), 0.0);
    std::fill (std::begin (a6LL), std::end (a6LL), 0.0); std::fill (std::begin (a6LR), std::end (a6LR), 0.0);
    std::fill (std::begin (a6ML), std::end (a6ML), 0.0); std::fill (std::begin (a6MR), std::end (a6MR), 0.0);
    std::fill (std::begin (a6NL), std::end (a6NL), 0.0); std::fill (std::begin (a6NR), std::end (a6NR), 0.0);
    std::fill (std::begin (a6OL), std::end (a6OL), 0.0); std::fill (std::begin (a6OR), std::end (a6OR), 0.0);
    std::fill (std::begin (a6PL), std::end (a6PL), 0.0); std::fill (std::begin (a6PR), std::end (a6PR), 0.0);
    std::fill (std::begin (a6QL), std::end (a6QL), 0.0); std::fill (std::begin (a6QR), std::end (a6QR), 0.0);
    std::fill (std::begin (a6RL), std::end (a6RL), 0.0); std::fill (std::begin (a6RR), std::end (a6RR), 0.0);
    std::fill (std::begin (a6SL), std::end (a6SL), 0.0); std::fill (std::begin (a6SR), std::end (a6SR), 0.0);
    std::fill (std::begin (a6TL), std::end (a6TL), 0.0); std::fill (std::begin (a6TR), std::end (a6TR), 0.0);
    std::fill (std::begin (a6UL), std::end (a6UL), 0.0); std::fill (std::begin (a6UR), std::end (a6UR), 0.0);
    std::fill (std::begin (a6VL), std::end (a6VL), 0.0); std::fill (std::begin (a6VR), std::end (a6VR), 0.0);
    std::fill (std::begin (a6WL), std::end (a6WL), 0.0); std::fill (std::begin (a6WR), std::end (a6WR), 0.0);
    std::fill (std::begin (a6XL), std::end (a6XL), 0.0); std::fill (std::begin (a6XR), std::end (a6XR), 0.0);
    std::fill (std::begin (a6YL), std::end (a6YL), 0.0); std::fill (std::begin (a6YR), std::end (a6YR), 0.0);
    std::fill (std::begin (a6ZAL), std::end (a6ZAL), 0.0); std::fill (std::begin (a6ZAR), std::end (a6ZAR), 0.0);
    std::fill (std::begin (a6ZBL), std::end (a6ZBL), 0.0); std::fill (std::begin (a6ZBR), std::end (a6ZBR), 0.0);
    std::fill (std::begin (a6ZCL), std::end (a6ZCL), 0.0); std::fill (std::begin (a6ZCR), std::end (a6ZCR), 0.0);
    std::fill (std::begin (a6ZDL), std::end (a6ZDL), 0.0); std::fill (std::begin (a6ZDR), std::end (a6ZDR), 0.0);
    std::fill (std::begin (a6ZEL), std::end (a6ZEL), 0.0); std::fill (std::begin (a6ZER), std::end (a6ZER), 0.0);
    std::fill (std::begin (a6ZFL), std::end (a6ZFL), 0.0); std::fill (std::begin (a6ZFR), std::end (a6ZFR), 0.0);
    std::fill (std::begin (a6ZGL), std::end (a6ZGL), 0.0); std::fill (std::begin (a6ZGR), std::end (a6ZGR), 0.0);
    std::fill (std::begin (a6ZHL), std::end (a6ZHL), 0.0); std::fill (std::begin (a6ZHR), std::end (a6ZHR), 0.0);
    std::fill (std::begin (a6ZIL), std::end (a6ZIL), 0.0); std::fill (std::begin (a6ZIR), std::end (a6ZIR), 0.0);
    std::fill (std::begin (a6ZJL), std::end (a6ZJL), 0.0); std::fill (std::begin (a6ZJR), std::end (a6ZJR), 0.0);
    std::fill (std::begin (a6ZKL), std::end (a6ZKL), 0.0); std::fill (std::begin (a6ZKR), std::end (a6ZKR), 0.0);
...
void KCathedral5::processStereo (const float* in1, const float* in2,
                                 float* out1, float* out2, int sampleFrames) noexcept
{
    ...
    double reg6n = (1.0 - std::pow (1.0 - A, 3.0)) * fdb6ck;
    ...
    derez = std::fmin (std::fmax (derez / overallscale, 0.0005), 1.0);
    ...
    { // this makes it match the 1.0 case using stepped
        derez /= (2.0 / std::pow (overallscale, 0.5 - ((overallscale - 1.0) * 0.0375)));
        bezTrim = 1.0 - std::pow (derez * 0.5, 1.0 / (derez * 0.5));
    ...
    derezFreq = std::fmin (std::fmax (derezFreq, 0.0005), 1.0); // note: no overallscale, already inside undersampling
    ...
    { // this makes it match the 1.0 case using stepped
        bezFreqTrim = 1.0 - std::pow (derezFreq * 0.5, 1.0 / (derezFreq * 0.5));
    } // the revision more accurately connects the bezier curves

    double earlyLoudness = D;
    int start = (int) (E * 27.0);
    int ld3G = kEarly[start];
    int ld3H = kEarly[start + 1];
    int ld3D = kEarly[start + 2];
    int ld3A = kEarly[start + 3];
    int ld3E = kEarly[start + 4];
    int ld3I = kEarly[start + 5];
    int ld3F = kEarly[start + 6];
    int ld3B = kEarly[start + 7];
    int ld3C = kEarly[start + 8];
    double wet = F;
    ...
    while (--sampleFrames >= 0)
    {
        ...
        if (bez[bez_cycle] > 1.0) // hit the end point and we do a reverb sample
        {
            ...
            double reverbInputSampleL = (o3GL + o3HL + o3IL) * 0.03125;
            double reverbInputSampleR = (o3AR + o3DR + o3GR) * 0.03125;

            double earlyReflectionL = reverbInputSampleL;
            double earlyReflectionR = reverbInputSampleR;

            bezF[bez_cycle] += derezFreq;
            bezF[bez_SampL] += ((reverbInputSampleL + bezF[bez_InL]) * derezFreq);
            // Original-Eigenheit: rechter Akkumulator nutzt reverbInputSampleL statt R (siehe Header/Kommentar oben)
            bezF[bez_SampR] += ((reverbInputSampleL + bezF[bez_InR]) * derezFreq);
            bezF[bez_InL] = reverbInputSampleL; bezF[bez_InR] = reverbInputSampleR;
            ...
            double Xfreq = bezF[bez_cycle] * bezFreqTrim;
            double CBLfreq = (bezF[bez_CL] * (1.0 - Xfreq)) + (bezF[bez_BL] * Xfreq);
            double BALfreq = (bezF[bez_BL] * (1.0 - Xfreq)) + (bezF[bez_AL] * Xfreq);
            double CBALfreq = (bezF[bez_BL] + (CBLfreq * (1.0 - Xfreq)) + (BALfreq * Xfreq)) * 0.0625;
            double CBRfreq = (bezF[bez_CR] * (1.0 - Xfreq)) + (bezF[bez_BR] * Xfreq);
            double BARfreq = (bezF[bez_BR] * (1.0 - Xfreq)) + (bezF[bez_AR] * Xfreq);
            double CBARfreq = (bezF[bez_BR] + (CBRfreq * (1.0 - Xfreq)) + (BARfreq * Xfreq)) * 0.0625;
            reverbInputSampleL = CBALfreq + bezF[bez_AvgOutSampL];
            bezF[bez_AvgOutSampL] = CBALfreq;
            reverbInputSampleR = CBARfreq + bezF[bez_AvgOutSampR];
            bezF[bez_AvgOutSampR] = CBARfreq;

            a6AL[c6AL] = reverbInputSampleL + (f6BL * reg6n);
            a6BL[c6BL] = reverbInputSampleL + (f6CL * reg6n);
            a6CL[c6CL] = reverbInputSampleL + (f6DL * reg6n);
            a6DL[c6DL] = reverbInputSampleL + (f6EL * reg6n);
            a6EL[c6EL] = reverbInputSampleL + (f6FL * reg6n);
            a6FL[c6FL] = reverbInputSampleL + (f6AL * reg6n);
            ...
            a6FR[c6FR] = reverbInputSampleR + (f6LR * reg6n);
            a6LR[c6LR] = reverbInputSampleR + (f6RR * reg6n);
            a6RR[c6RR] = reverbInputSampleR + (f6XR * reg6n);
            a6XR[c6XR] = reverbInputSampleR + (f6ZER * reg6n);
            a6ZER[c6ZER] = reverbInputSampleR + (f6ZKR * reg6n);
            a6ZKR[c6ZKR] = reverbInputSampleR + (f6FR * reg6n);
            ...
            reverbInputSampleL = (o6ZFL + o6ZGL + o6ZHL + o6ZIL + o6ZJL + o6ZKL) * 0.001953125;
            reverbInputSampleR = (o6AR + o6GR + o6MR + o6SR + o6YR + o6ZFR) * 0.001953125;

            f6AL = (f6AL + f6AL + f6AL + std::fabs (avg6L)) * 0.25; avg6L = f6AL;
            f6FR = (f6FR + f6FR + f6FR + std::fabs (avg6R)) * 0.25; avg6R = f6FR;
            // manipulating deep reverb tail for realism

            reverbInputSampleL += (earlyReflectionL * earlyLoudness);
            reverbInputSampleR += (earlyReflectionR * earlyLoudness);

            bez[bez_CL] = bez[bez_BL];
            bez[bez_BL] = bez[bez_AL];
            bez[bez_AL] = reverbInputSampleL;
            bez[bez_SampL] = 0.0;

            bez[bez_CR] = bez[bez_BR];
            bez[bez_BR] = bez[bez_AR];
            bez[bez_AR] = reverbInputSampleR;
            bez[bez_SampR] = 0.0;
        }
