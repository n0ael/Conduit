 ...
 *  ----------------------------------------
 *  Conduit-Port aus plugins/LinuxVST/src/kBeyond (github.com/airwindows/airwindows) —
...
namespace
{
    constexpr ParameterInfo kParameters[] = {
        ...
        { "predlay", "Predlay", 0.0f },
        { "drywet",  "Dry/Wet", 0.5f },
    };
}

KBeyond::KBeyond() noexcept
    : AirwindowsPlugin ("kbeyond", "kBeyond", kParameters)
{
}

void KBeyond::resetState() noexcept
{
    ...
    std::fill (std::begin (firBufferL), std::end (firBufferL), 0.0);
    std::fill (std::begin (firBufferR), std::end (firBufferR), 0.0);
    firPosition = 0;
}

void KBeyond::processStereo (const float* in1, const float* in2,
                             float* out1, float* out2, int sampleFrames) noexcept
{
    ...
    // Original: derez = 1.0/((int)(1.0/derez)) — bei derez==0 (Parameter B==0)
    // ist 1.0/derez == +inf und die (int)-Konvertierung UB (Conduit-Hardregel
    // 3.1 verbietet nicht nur Allocation, sondern generell UB im Audio-Pfad).
    // Guard behält das Original-Ergebnis für derez>0 exakt bei (identische
    // Rechnung) und trifft für derez==0 direkt den folgenden Clamp auf
    // 0.0005 — denselben Wert, auf den das Original auf x86/SSE2 durch die
    // definierte "integer indefinite" Konvertierung (INT_MIN) ohnehin
    // zusteuert, nur ohne die UB-Abhängigkeit.
    derez = (derez > 0.0) ? 1.0 / (double) (int) (1.0 / derez) : 0.0;
    if (derez < 0.0005) derez = 0.0005;
    if (derez > 1.0) derez = 1.0;
    double freq = C * juce::MathConstants<double>::halfPi; if (freq < 0.5) freq = 0.5;
    double earlyLoudness = D;
    int adjPredelay = (int) (predelay * E * derez);
    double wet = F;
    ...
    while (--sampleFrames >= 0)
    {
        ...
        if (bez[bez_cycle] > 1.0) // hit the end point and we do a reverb sample
        {
            ...
            double reverbInputSampleL = (o3GL + o3HL + o3IL) * 0.125;
            double reverbInputSampleR = (o3AR + o3DR + o3GR) * 0.125;
            ...
            firBufferL[firp] = reverbInputSampleL; reverbInputSampleL = 0.0;
            firBufferR[firp] = reverbInputSampleR; reverbInputSampleR = 0.0;
            if (firp + 48 < 32767)
            {
                for (int fip = 1; fip < 48; fip++)
                {
                    reverbInputSampleL += firBufferL[firp + fip] * fir[fip];
                    reverbInputSampleR += firBufferR[firp + fip] * fir[fip];
                }
            }
            else
            {
                for (int fip = 1; fip < 48; fip++)
                {
                    reverbInputSampleL += firBufferL[firp + fip - ((firp + fip > 32767) ? 32768 : 0)] * fir[fip];
                    reverbInputSampleR += firBufferR[firp + fip - ((firp + fip > 32767) ? 32768 : 0)] * fir[fip];
                }
            }
            reverbInputSampleL *= 0.25; reverbInputSampleR *= 0.25;
            reverbInputSampleL *= std::sqrt (freq); reverbInputSampleR *= std::sqrt (freq);
